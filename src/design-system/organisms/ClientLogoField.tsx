...
export const ClientLogoField: React.FC<ClientLogoFieldProps> = ({
  ...
  const bgClass = light ? 'bg-[#173C62] text-white' : 'bg-[#F8FAFC] text-[#0B1320]';
    ...
    <section className={`py-14 sm:py-20 ${bgClass} ${className}`}>
      <Container className="space-y-12">
        <div className="max-w-2xl mx-auto text-center space-y-3">
          <span className={`font-mono text-xs uppercase tracking-[0.2em] ${light ? 'text-slate-300' : 'text-[#173C62]'}`}>
            {eyebrow}
          </span>
