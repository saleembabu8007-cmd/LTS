...
export const AsymmetricMediaMosaic: React.FC<AsymmetricMediaMosaicProps> = ({
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-12 gap-6 lg:gap-8 ${className}`}>
      ...
      {items[0] && (
        <div className="md:col-span-7 relative group overflow-hidden rounded-[28px] lg:rounded-[32px] aspect-[4/3] md:aspect-[16/11] bg-slate-100">
          <img
            src={items[0].imageSrc}
            alt={items[0].imageAlt}
            className="w-full h-full object-cover object-center rounded-[28px] lg:rounded-[32px] transition-transform duration-700 group-hover:scale-[1.03]"
          />
      ...
      <div className="md:col-span-5 flex flex-col gap-6 lg:gap-8">
        {items.slice(1, 3).map((item, idx) => (
          <div
            key={idx}
            className="relative group overflow-hidden rounded-[24px] lg:rounded-[28px] aspect-[16/10] md:aspect-[16/9] flex-1 bg-slate-100"
          >
            <img
              src={item.imageSrc}
              alt={item.imageAlt}
              className="w-full h-full object-cover object-center rounded-[24px] lg:rounded-[28px] transition-transform duration-700 group-hover:scale-[1.03]"
            />
