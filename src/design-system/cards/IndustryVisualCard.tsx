...
export const IndustryImageCard: React.FC<IndustryCardProps> = ({
  ...
  if (variant === 'overlay') {
    return (
      <a
        href={href}
        onClick={handleClick}
        className={`group relative block w-full select-none text-left overflow-hidden rounded-[8px] bg-[#173C62] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#173C62] focus-visible:ring-offset-4 ${className}`}
  ...
  return (
    <a
      href={href}
      onClick={handleClick}
      className={`group block select-none text-left rounded-[8px] transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#173C62] focus-visible:ring-offset-4 ${className}`}
      aria-label={`View sector capabilities for ${title}`}
    >
      <div className={`relative w-full overflow-hidden rounded-[8px] bg-[#173C62] ${aspectClass}`}>
        <img
          src={imageUrl}
          alt={imageAlt || title}
          loading="lazy"
