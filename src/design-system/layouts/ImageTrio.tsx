    ...
    <div className={`grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8 items-center ${className}`}>
      ...
      <div className={`md:col-span-7 ${reverse ? 'md:order-2' : 'md:order-1'}`}>
        <div className="relative aspect-[4/3] overflow-hidden rounded-[18px] bg-[#173C62]">
          ...
          />
          {primary.caption && (
            <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-[#173C62]/85 to-transparent p-4 text-[12px] font-mono text-white/90">
              {primary.caption}
            </div>
          )}
        </div>
      ...
      <div className={`md:col-span-5 flex flex-col gap-6 md:gap-8 ${reverse ? 'md:order-1' : 'md:order-2'}`}>
        <div className="relative aspect-[16/10] overflow-hidden rounded-[18px] bg-[#173C62]">
          ...
          />
          {secondary1.caption && (
            <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-[#173C62]/85 to-transparent p-3 text-[11px] font-mono text-white/90">
              {secondary1.caption}
            </div>
          )}
        </div>
